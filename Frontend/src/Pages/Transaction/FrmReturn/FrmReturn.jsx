    ...
    <div className="flex flex-col">
      ...
      {options && options.length > 0 ? (
        <select
          ...
          className="border rounded-md p-2 text-sm focus:ring-2 focus:ring-blue-500 transition duration-150 ease-in-out"
          {...props}
        >
        ...
        const data = res.data.map((data) => {
          return [
            ...
            <div className="flex justify-center items-centr">
              <Link
                to={`/Transaction/FrmReturnForm`}
                state={{
                  returnAdjId: data.returnAdjId,
                }}
      ...
      {transaction.actionNeeded ? (
        <>
          <Button
            ...
            onClick={() => {
              // Logic to update the status to "Completed"
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 space-y-4 md:space-y-0">
        <Button
          ...
          onClick={() => navigate("/Transaction/FrmReturnForm")}
        >
          Return Stocks
        </Button>
      </div>
      <Table
        // headerlabel="Return Transactions List"
        headers={headers}
        data={tableData}
      />
    </Layout>
